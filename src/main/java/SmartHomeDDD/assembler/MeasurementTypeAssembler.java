package SmartHomeDDD.assembler;

import SmartHomeDDD.DTO.MeasurementTypeDTO;
import SmartHomeDDD.ddd.Assembler;
import SmartHomeDDD.domain.Unit.Unit;

import java.util.ArrayList;
import java.util.List;

/**
 * Assembler class for converting {@link Unit} domain entities to {@link MeasurementTypeDTO} data transfer objects.
 */

public class MeasurementTypeAssembler implements Assembler<Unit, MeasurementTypeDTO> {
    /**
     * Constructor of the MeasurementTypeAssembler class.
     */
    public MeasurementTypeAssembler() {

    }

    /**
     * Converts a {@link Unit} domain entity to a {@link MeasurementTypeDTO} data transfer object.
     *
     * @param unit is the domain entity to be converted.
     * @return The {@link MeasurementTypeDTO} data transfer object.
     */
    @Override
    public MeasurementTypeDTO domainToDTO(Unit unit) {
        if (unit == null) {
            throw new IllegalArgumentException("The MeasurementType cannot be null.");
        }
        String measurementID = unit.getID().toString();
        String measurementTypeDescription = unit.getUnitDescription().toString();

        return new MeasurementTypeDTO(measurementID, measurementTypeDescription);
    }

    /**
     * Converts a list of {@link Unit} domain entities to a list of {@link MeasurementTypeDTO} data transfer objects.
     *
     * @param units is the list of domain entities to be converted.
     * @return
     */

    @Override
    public List<MeasurementTypeDTO> domainToDTO(List<Unit> units) {
        if (units == null) {
            throw new IllegalArgumentException("The list of MeasurementTypes cannot be null.");
        }
        List<MeasurementTypeDTO> measurementTypeDTOS = new ArrayList<>();
        for (Unit unit : units) {
            MeasurementTypeDTO measurementTypeDTO = domainToDTO(unit);
            measurementTypeDTOS.add(measurementTypeDTO);
        }
        return List.copyOf(measurementTypeDTOS);
    }


}
