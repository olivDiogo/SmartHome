...
class GetLogFromDeviceControllerTest {
  ...
  void shouldGetRoomsFromHouse_WhenGivenValidHouseID() {
    ...
    List<String> expectedList = List.of(expectedRoomDTOName1, expectedRoomDTOID1);
    ...
    List<String> actualList = List.of(actualRoomDTOName1, actualRoomDTOID1);
  ...
  void shouldThrowException_WhenRoomIDDoesNotExistInRepository() {
    ...
    RoomDTO roomDTO =
        new RoomDTO(
            houseID.toString(), roomName1.toString(), dimension.toString(), roomFloor.toString());

    // Act & Assert
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> getLogFromDeviceController.getDevicesFromRoom(roomDTO));
    assertEquals(
        "Room with ID " + new RoomID(roomDTO.roomId) + " not found.", exception.getMessage());
  }
  ...
  void shouldGetDevicesFromRoom_WhenParametersAreValid() {
    ...
    Room room = roomService.addRoom(houseID, roomName2, dimension, roomFloor);
